...
- (BOOL)startRecording {
    ...
    // Configure audio session for recording (puts session in PlayAndRecord temporarily)
    Monke_SetAudioSessionRecord();
    ...
    self.recognitionTask = [self.speechRecognizer recognitionTaskWithRequest:self.recognitionRequest resultHandler:^(SFSpeechRecognitionResult *result, NSError *error) {
        ...
        if (error || isFinal) {
            ...
            // Return to playback session once we're done listening
...
- (void)onSilenceTimeout {
    ...
    // Return to playback session so video/audio/TTS stay smooth
    Monke_SetAudioSessionPlayback();
}
...
- (void)stopRecording {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        // Restore playback-focused session after manual stop
        Monke_SetAudioSessionPlayback();
    });
}
